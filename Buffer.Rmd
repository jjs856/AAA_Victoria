...
# Try to join GPS participants and buffer and see what % is inside buffer
```{r}
join_gps_to_buffer <- lapply(sd_data, function(i) {
  i %>%
...
# figure out what buf is, and make it. 
```{r}
buf_intersections <- st_intersection(buf$geometry, pandora$geometry) #this works now and hasn’t worked before until today
...
# Create dummy variable for 2019 bike lanes - Do after intersection for Pandora 2017 is done
```{r}
