...
# apply 70m buffer
w2_intervention_buffer = st_buffer(w2_intervention_1$x, dist = 70) # According to stack exchangers the distance is in meters
...
SD_vic_one_sf$exposed <- ifelse(sf::st_intersects(SD_vic_one_sf, pandora_buffer, sparse = F), "Yes", "No")

table(SD_vic_one_sf$exposed)
SD_vic_one$exposed <- as.factor(SD_vic_one_sf$exposed)

ggplot() + 
        geom_sf() +
        geom_sf(data = pandora_buffer) +                
        geom_point(aes(x = lon, y = lat, colour = exposed), data = SD_vic_one, alpha = 0.2) +
...
```{r}
w2_intervention_buffer <- st_transform(w2_intervention_buffer, 4326)
w2_intervention_buffer <- st_as_sf(w2_intervention_buffer)

SD_vic_two_sf$exposed <- ifelse(sf::st_intersects(SD_vic_two_sf, w2_intervention_buffer, sparse = F), "Yes", "No")

table(SD_vic_two_sf$exposed)
SD_vic_two$exposed <- as.factor(SD_vic_two_sf$exposed)

ggplot() + 
        geom_sf() +
        geom_sf(data = w2_intervention_buffer) +                
        geom_point(aes(x = lon, y = lat, colour = exposed), data = SD_vic_two, alpha = 0.2) +
        geom_sf(data = w2_intervention_buffer) +
        annotation_scale() + 
        theme_map()
...
table(sd_data_exposure$wave_id, sd_data_exposure$exposed)
```
