...
```{r}
vh_1 %>% mutate(participation = 1) # Add variable to victoria health w1 (1)
ve_1 %>% mutate(participation = 1) # Add variable to victoria eligibility w1 (1)

vh_2 %>% mutate(participation = 2.5)# Add variable for vh_2 returning 2 = (2.5)
vh_2_new %>% mutate(participation = 2) # Add variable for vh_2_new = (2)

ve_2 %>% mutate(participation = 2.5) # Add variable for ve_2  = (2.5)
ve_2_new %>% mutate(participation = 2) # Add a variable for ve_2_new (2)
```
...
# Merge w1
w1 <- full_join(vh_1, ve_1) # Got all 281 participants, but delete the 27 duds
w1 <- w1[-(282:403),]

# Merge w2 e and h
w2e <- full_join(ve_2, ve_2_new)

#w2e <- w2e[-(282:315),]
w2h <- full_join(vh_2, vh_2_new)

## Now what? haha
# bind_rows just stacks everything, doesn't combine data for one participant (2-3 rows per participant)
# full_join by "interact_id" returns n=405 with data for only the first 281.
# left_join retains n = 281 or right_join only retains the n= 287. 
...
vm <- bind_rows(w1, w2h, w2e, .id = "interact_id") #merging
